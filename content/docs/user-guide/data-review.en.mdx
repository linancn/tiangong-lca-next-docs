...
![Data check button](/assets/docs/0a311ce8/data-check.png)

If the platform finds blocking issues during submission, it opens a problem list. Fix those issues
first, then submit again.

![Submission validation prompt](/assets/docs/956dc785/data-problem.png)

For process datasets, **Submit for review** first saves the current revision and checks its structure,
quantitative reference, and reference integrity. When those synchronous checks pass, it calls review
submission directly; there is no separate numerical-stability-gate task to await in Task Center. The
server may still reject the command for authorisation, a non-current version, a conflicting review,
or concurrent state. Fix the reported condition, save, and resubmit.

A review admin can manually run the completeness and numerical-stability quality diagnostic. Its
report is informational: findings, a `not_evaluable` outcome, or a failed run do not change review
state or block assignment, approval, rejection, or resubmission.
...
| Team admin | Coordinate with authors and help track project progress | My Team, [notification center](/en/docs/user-guide/notifications/) |

> Review members submit advisory opinions. Only a review admin can finally approve or reject the
> whole review. Review roles and team roles are different permission systems. A person may have both, or only one
> of them.
...
![Review member workspace tabs](/assets/docs/8e9c116a/review-workspace-member-tabs.png)
...
Also watch the **Data Notifications** tab in the top-right [notification center](/en/docs/user-guide/notifications/) so
...
![Review status filters](/assets/docs/9fa6d4a0/review-status.png)
...
Continue with [Permissions & Data Spaces](/en/docs/user-guide/permissions-and-data-scopes/).
