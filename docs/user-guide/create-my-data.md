...
首次研制过程数据时，可以先在登录后的欢迎页点击 **数据研制指南**。该页面会展示
**天工生命周期数据库**说明、**操作演示视频**、**过程数据研制流程**步骤和数据对象说明，
适合在正式进入“我的数据”之前确认数据准备顺序。

![欢迎页中的数据研制指南展示了数据库说明、操作演示视频、过程数据研制流程和数据对象卡片。](./img/welcome-data-development-guide.png)
...
2. **选择面板**：遇到需要选择的数据（如“数据集格式”“合规系统名称”）时：  
   ...
   - 点击底部“保存”确认。  
   ![](./img/choose.png)
3. **版本号管理**：首次新建数据集时按页面要求填写或确认版本号，并定期点击“保存”防止网络异常导致的数据丢失。若从“所有版本”进入“新增版本”，版本字段会显示“新版本自动计算后生成”，系统会基于所选源版本自动生成下一版本号，请不要手动改写版本号。
...
平台支持自定义单位组与流属性，但自定义配置会影响后续 LCIA 计算，请谨慎评估再使用。当前“我的数据”中的单位组和流属性新增、导入、复制等维护入口仅系统管理员可用；如果页面提示需要新增或补充，请联系管理员。

### 新建单位组

1. 如果您是系统管理员，打开“我的数据”→“单位组”，点击 `+`。普通用户如需补充单位组，请联系管理员。  
...
1. 如果您是系统管理员，打开“我的数据”→“流属性”，点击 `+`。普通用户如需补充流属性，请联系管理员。  
