...
- openLCA JSON-LD exchange amounts are normalised to each flow's reference flow property reference unit. For example, when a source exchange is stated in `g` or another flow-property unit, the converted TIDAS exchange amount is written in the reference unit while the original amount, unit, and flow property remain in trace. If a unit cannot be resolved, the conversion report marks that exchange as not normalised.
