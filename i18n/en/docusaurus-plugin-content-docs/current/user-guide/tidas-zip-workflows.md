...
- Exchange amounts from openLCA JSON-LD are normalised where possible to the reference unit of the flow's reference flow property. The original amount, unit, and flow property stay in the trace for source-data review.
- If some exchange amounts cannot be normalised, `conversion-report.json` records the unresolved reason. Review those warnings before import so non-reference-unit values are not mistaken for final amounts.
